...
hoverables.forEach(hoverable => {
	if (!(hoverable instanceof HTMLElement)) return;

	const edges = { left: "left", top: "top", bottom: "bottom", right: "right" };

	const handleMouseEvent = (e: MouseEvent, type: "enter" | "exit") => {
...
function closestEdge<T>(mouse: MouseEvent, elem: HTMLElement, edges: ClosestEdges<T>): T {
