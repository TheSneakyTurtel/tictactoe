...
if (mainContent && sidebar) {
    ...
    let prevSidebarSelector = undefined;
    sidebar.addEventListener("click", e => {
        ...
        if (sidebarSelector instanceof HTMLElement && !isNaN(parseInt(sidebarSelector.dataset.sectionIndex)) && prevSidebarSelector !== sidebarSelector) {
            ...
            if (prevSidebarSelector)
                prevSidebarSelector.classList.remove("active");
            else
                for (let i = 0; i < sidebarSelector.parentElement.children.length; i++)
                    sidebarSelector.parentElement.children[i].classList.remove("active");
